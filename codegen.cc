...
CodeGenerator::CodeGenerator() { curGlobalOffset = 0; }
...
Location *CodeGenerator::GenBinaryOp(const char *opName, Location *op1,
  ...
  Location *result = GenTempVar();
  code.push_back(
      new BinaryOp(BinaryOp::OpCodeForName(opName), result, op1, op2));
  return result;
}
...
BeginFunc *CodeGenerator::GenBeginFunc(FnDecl *fn) {
  ...
  for (int i = 0; i < formals->NumElements(); i++)
    formals->Nth(i)->rtLoc = new Location(fpRelative, i * VarSize + start,
...
Location *CodeGenerator::GenFunctionCall(const char *fnLabel,
                                         List<Location *> *args,
                                         bool hasReturnValue) {
  for (int i = args->NumElements() - 1; i >= 0;
       i--) // push params right to left
...
Location *CodeGenerator::GenMethodCall(Location *rcvr, Location *meth,
                                       List<Location *> *args,
    ...
    while (p != code.end()) {
      ...
      if (dynamic_cast<BeginFunc *>(*p)) {
        ...
        if (p == code.end())
          ...
          // use CFG for liveness analysis
          ControlFlowGraph cfg(begin_func, end_func);
          LiveVariable *analysis = new LiveVariable(cfg);
          analysis->analyze();
          mips.SetAnalysis(analysis);
...
Location *CodeGenerator::GenDynamicDispatch(Location *rcvr, int vtableOffset,
                                            List<Location *> *args,
                                            bool hasReturnValue) {
